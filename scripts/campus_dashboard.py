...
class EnergySample:
    def _init_(self, ts, value):
        self.time = pd.to_datetime(ts)
        self.value = float(value)


class Facility:
    def _init_(self, label: str):
        self.label = label
...
class FacilityCollection:
    ...
    def import_csv_dir(self, folder: Path):
        ...
        if not csvs:
            logging.warning(
                f"No CSV files found in directory: {folder.resolve()}")
        ...
        for file in csvs:
            try:
                ...
                df = pd.read_csv(file, parse_dates=[
                                 "timestamp"], on_bad_lines="skip")
                if not {"timestamp", "kwh"}.issubset(df.columns):
                    logging.error(
                        f"Missing required columns in {file}. Skipped.")
                    continue

                label = file.stem.replace("usage", "").replace("", " ").title()
...
def draw_dashboard(all_df, daily_df, weekly_df, summary_df, manager):
    ...
    fig, ax = plt.subplots(3, 1, figsize=(13, 15), gridspec_kw={
                           "height_ratios": [1, 1, 1]})

    # --- Chart 1: Rolling Daily Usage --- #
    ax0 = ax[0]
    for name, grp in all_df.groupby("building"):
        roll = grp.set_index("timestamp").resample(
            "D")["kwh"].sum().rolling(7, min_periods=1).mean()
        ax0.plot(roll.index, roll.values, label=name)
    ax0.set_title("7-Day Rolling Energy Usage")
    ax0.legend()

    # --- Chart 2: Weekly Averages --- #
    weekly_avg = weekly_df.groupby(
        "building")["kwh"].mean().sort_values(ascending=False)
